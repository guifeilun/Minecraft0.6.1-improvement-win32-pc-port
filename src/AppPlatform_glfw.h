...
public:
	...
	virtual void toggleFullscreen() override {
		...
		if (glfwGetWindowMonitor(window) != NULL) {
			// Currently fullscreen — restore the saved windowed geometry.
			glfwSetWindowMonitor(window, NULL,
				_windowedX, _windowedY, _windowedW, _windowedH, 0);
		} else {
			// Save current position/size, then go fullscreen on the primary monitor.
			glfwGetWindowPos(window, &_windowedX, &_windowedY);
			glfwGetWindowSize(window, &_windowedW, &_windowedH);
...
private:
	int _windowedX = 0, _windowedY = 0, _windowedW = 800, _windowedH = 600;
};
