...
OptionInt fov("fov", 70, 30, 110);

OptionBool forceNether("forceNether", false);
...
// Java-style brightness/gamma. 0.0 = "Moody" (no boost, original ramp),
// 1.0 = "Bright!" (lowest light level is lifted to ~0.5). Bucketed into
...
void Options::initTable() {
	...
	m_options[OPTIONS_FOV] = &fov;

	m_options[OPTIONS_FORCE_NETHER] = &forceNether;
	...
	m_options[OPTIONS_BRIGHTNESS] = &gamma;
...
void Options::load() {
	...
	for (auto i = 0; i < optionStrings.size(); i += 2) {
		...
		// A few options have global side-effects (light ramp gamma, etc.) that
		// need to take effect immediately after load — not just when the user
		// touches the slider. Notify them so optionUpdated() reapplies.
		if ((*opt) == m_options[OPTIONS_BRIGHTNESS] && minecraft) {
