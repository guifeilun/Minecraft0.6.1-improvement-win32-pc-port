...
void Item::initItems() {
	...
	// Phase 1 wire-up: these were originally commented out because their custom
	// item classes (BucketItem, MinecartItem, BoatItem, FishingRodItem, etc.)
	// don't exist as functional implementations. We register them as plain
	// Item placeholders so they get an id, icon, and inventory slot — clicking
	// them does nothing useful yet, but they're craftable, collectable, and
	// listed in the creative menu.
	Item::apple_gold = (new FoodItem(66, 10, false))->setIcon(11, 0)->setCategory(ItemCategory::FoodArmor)->setDescriptionId("appleGold");
	...
	// Buckets — real BucketItem instances with right-click liquid pick-up /
	// placement. Contents are referenced by tile id, so they must be wired
	// AFTER Tile::initTiles() has run.
	Item::bucket_empty = (new BucketItem(69, 0))->setIcon(10, 4)->setCategory(ItemCategory::Tools)->setDescriptionId("bucket");
	Item::bucket_water = (new BucketItem(70, Tile::calmWater ? Tile::calmWater->id : 0))->setIcon(11, 4)->setCategory(ItemCategory::Tools)->setDescriptionId("bucketWater")->setCraftingRemainingItem(Item::bucket_empty);
	Item::bucket_lava  = (new BucketItem(71, Tile::calmLava  ? Tile::calmLava->id  : 0))->setIcon(12, 4)->setCategory(ItemCategory::Tools)->setDescriptionId("bucketLava")->setCraftingRemainingItem(Item::bucket_empty);
	Item::minecart = (new Item(72))->setMaxStackSize(1)->setIcon(7, 8)->setCategory(ItemCategory::Mechanisms)->setDescriptionId("minecart");
	Item::saddle = (new Item(73))->setMaxStackSize(1)->setIcon(8, 6)->setCategory(ItemCategory::Mechanisms)->setDescriptionId("saddle");
	Item::door_iron = (new DoorItem(74, Material::metal))->setIcon(12, 2)->setCategory(ItemCategory::Structures)->setDescriptionId("doorIron");
	Item::redStone = (new TilePlanterItem(75, Tile::redStoneDust))->setIcon(8, 3)->setCategory(ItemCategory::Mechanisms)->setDescriptionId("redstone");
	Item::snowBall = (new SnowballItem(76))->setIcon(14, 0)->setCategory(ItemCategory::Decorations)->setDescriptionId("snowball");
	Item::boat = (new Item(77))->setMaxStackSize(1)->setIcon(8, 8)->setCategory(ItemCategory::Mechanisms)->setDescriptionId("boat");
	Item::leather = (new Item(78))->setIcon(7, 6)->setCategory(ItemCategory::Tools)->setDescriptionId("leather");
	Item::milk = (new Item(79))->setMaxStackSize(1)->setIcon(13, 4)->setCategory(ItemCategory::FoodArmor)->setDescriptionId("milk");
	...
	Item::minecart_chest = (new Item(86))->setMaxStackSize(1)->setIcon(7, 9)->setCategory(ItemCategory::Mechanisms)->setDescriptionId("minecartChest");
	Item::minecart_furnace = (new Item(87))->setMaxStackSize(1)->setIcon(7, 10)->setCategory(ItemCategory::Mechanisms)->setDescriptionId("minecartFurnace");
	...
	Item::fishingRod = (new FishingRodItem(90))->setMaxStackSize(1)->setIcon(5, 4)->setCategory(ItemCategory::Tools)->setDescriptionId("fishingRod");
	...
	Item::fish_raw = (new FoodItem(93, 2, false))->setIcon(9, 5)->setCategory(ItemCategory::FoodArmor)->setDescriptionId("fishRaw");
	Item::fish_cooked = (new FoodItem(94, 5, false))->setIcon(10, 5)->setCategory(ItemCategory::FoodArmor)->setDescriptionId("fishCooked");
	...
	Item::diode = (new Item(100))->setIcon(6, 5)->setCategory(ItemCategory::Mechanisms)->setDescriptionId("diode");
