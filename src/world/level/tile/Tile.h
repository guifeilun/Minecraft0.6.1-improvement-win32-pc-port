...
public:
	...
	// Redstone lamp — two ids, swapped by the lamp's own tick. Off form
	// is the one the item economy exposes; the on form is a runtime-only
	// variant the lamp tile switches itself into when neighbours have
	// any redstone signal.
	static Tile* redStoneLamp_off;
	static Tile* redStoneLamp_on;
