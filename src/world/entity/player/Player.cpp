...
// 10% faster than vanilla 0.1 — applies to walking; sprint multiplies this
// (LocalPlayer::getWalkingSpeedModifier returns 1.3 when sprinting) so sprint
// also gains the same 10%.
const float Player::DEFAULT_WALK_SPEED = 0.11f;
	...
	allPlayersSleeping(false)
{
	...
	// 10% faster walking than the Mob default. Sprint multiplier (1.3x in
	// LocalPlayer::getWalkingSpeedModifier) compounds on top, so sprint speed
	// also gets the same 10% boost.
